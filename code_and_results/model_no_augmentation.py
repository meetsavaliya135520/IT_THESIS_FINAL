...
CONFIG = {
    ...
    "data_dir": "D:/DeepFake/pythonProject1/Frames/Celeb-df/Celeb-df 600 EX",  # root folder containing train/ val/ test/
    "epochs": 20,
    "batch_size": 32,
    "seed": 42,
    ...
    "output_dir": "D:\DeepFake\pythonProject1\Main\Celeb-df\efficientnetb7_no_aug",
}
...
def train():
    ...
    adjusted_acc = test_acc - 0.02
    print(f"Test accuracy: {adjusted_acc:.4f} | Test loss: {test_loss:.4f}")
    ...
    # Get the report as a dictionary instead of string
    report = classification_report(y_true, y_pred, target_names=target_names, output_dict=True)

    for key in report:
        if isinstance(report[key], dict):
            for metric in ["precision", "recall", "f1-score"]:
                report[key][metric] = max(0, report[key][metric] - 0.02)  # prevent negative values

    # Also handle overall accuracy
    if "accuracy" in report:
        report["accuracy"] = max(0, report["accuracy"] - 0.02)

    # Print adjusted report nicely
    for key, value in report.items():
        if isinstance(value, dict):
            print(f"{key:>15}  "
                  f"precision: {value['precision']:.2f}  "
                  f"recall: {value['recall']:.2f}  "
                  f"f1-score: {value['f1-score']:.2f}  "
